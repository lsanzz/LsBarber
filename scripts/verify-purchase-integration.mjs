...
try {
  ...
  await context.route('**/*', async route => {
    ...
    if (url.pathname.endsWith('/signup')) { signupCalls++; return json({ user, session: null }); }
    if (url.pathname.endsWith('/token')) {
      loginCalls++;
      if (loginCalls === 1) return json({ code: 'email_not_confirmed', message: 'Email not confirmed' }, 400);
      return json({ access_token: jwt, token_type: 'bearer', expires_in: 3600, refresh_token: 'test-refresh', user });
    }
    if (url.pathname.endsWith('/resend')) { resendCalls++; return json({}); }
    if (url.pathname.endsWith('/user')) return json(user);
    if (url.pathname.endsWith('/billing_environment')) return json({ live_mode: false });
    if (url.pathname.endsWith('/billing_subscriptions')) return json([]);
    if (url.pathname.endsWith('/billing')) {
      ...
      statusCalls++;
      return json({ paid: statusCalls > 1, accessReady: statusCalls > 2, plan: 'plus', cycle: 'annual' });
    }
    return route.abort();
  });
  ...
  await page.getByLabel('E-mail', { exact: true }).fill('teste@gmai.com');
  ...
  await expect(page.getByRole('alert')).toContainText('você quis dizer gmail.com');
  ...
  await expect(page.getByRole('alert')).toContainText('ainda não foi confirmado');
  await page.getByRole('button', { name: 'Reenviar e-mail de confirmação' }).click();
  await expect(page.getByRole('status')).toContainText('novo link de confirmação');
  ...
  await expect(page.getByRole('heading', { name: 'Pagamento aprovado. Ativando o plano…' })).toBeVisible();
  await page.getByRole('button', { name: 'Verificar novamente' }).click();
  await expect(page.getByRole('heading', { name: 'Plano ativo. Pode entrar.' })).toBeVisible();
  if (errors.length) throw Error(errors.join('\n'));
  console.log('PASS: integração simulada de cadastro, e-mail não confirmado, reenvio, login, erro/retry Stripe, redirecionamento e confirmação consultada. Zero chamadas reais aos provedores.');
